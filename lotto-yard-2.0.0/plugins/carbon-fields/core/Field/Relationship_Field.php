...
class Relationship_Field extends Field
{
    ...
    public function template()
    {
		...
		<div class="relationship-container">
			<div class="selected-items-container">
				<strong>
					...
					<span class="selected-label" data-single-label="<?php _e('selected item', 'carbon_fields');
        ?>" data-plural-label="<?php _e('selected items', 'carbon_fields');
        ?>">
        ...
        ?>
                    ...
                    ?>
					<# if ( max !== -1 ) { #>
						<span class="remaining"><?php _e('out of', 'carbon_fields');
        ?> {{{ max }}}</span>
					<# } #>
				</strong>
        ...
        ?>
			...
			<div class="relationship-right">
				<label><?php _e('Associated:', 'carbon_fields');
        ?></label>
    ...
    public function item_template($display_input = true)
    {
		...
		<li>
			...
			<a href="#" data-item-id="{{{ item.id }}}" data-item-title="{{{ item.title }}}" data-item-type="{{{ item.type }}}" data-item-subtype="{{{ item.subtype }}}" data-item-label="{{{ item.label }}}" data-value="{{{ item.id }}}">
				<# if ( item.edit_link ) { #>
					<em class="edit-link" data-href="{{{ item.edit_link }}}"><?php _e('Edit', 'carbon_fields');
        ?></em>
				<# } #>
				<em>{{{ item.label }}}</em>
				<span></span>
